...
function generateClashConfig(parsedLinks: V2RayLink[], options: ConfigOptions = {}): string {
  ...
  let config = `# Generated by subconvert.web.id\n# Date: ${new Date().toISOString()}\n`;
  ...
  if (isFullConfig) {
    ...
    if (adsBlock) {
      config += `  - name: "BLOCK-ADS"\n    type: select\n    proxies:\n      - REJECT\n      - DIRECT\n`;
      if (bestPing) config += `      - BEST-PING\n`;
      if (loadBalance) config += `      - LOAD-BALANCE\n`;
      if (fallback) config += `      - FALLBACK\n`;
    }
    if (pornBlock) {
      config += `  - name: "BLOCK-PORN"\n    type: select\n    proxies:\n      - REJECT\n      - DIRECT\n`;
      if (bestPing) config += `      - BEST-PING\n`;
      if (loadBalance) config += `      - LOAD-BALANCE\n`;
      if (fallback) config += `      - FALLBACK\n`;
    }
    ...
    config += `rules:\n`;
    if (adsBlock) {
      config += `  - RULE-SET,BLOCK-ADS,BLOCK-ADS\n`;
    }
    if (pornBlock) {
      config += `  - RULE-SET,BLOCK-PORN,BLOCK-PORN\n`;
    }
