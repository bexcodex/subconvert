...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function convertV2rayToConfig(v2rayLinks) {
    ...
    const isWildcardMode = wildcardBtn.classList.contains("active");
    ...
    if (useCustomServer && customServerValue) {
        parsedLinks.forEach(link => {
            ...
            link.server = customServerValue;

            if (isWildcardMode) {
                // Wildcard Mode: servername/sni/host = bughost.domainasli
                // Append customServerValue as prefix to original domain
                ...
                // For GRPC - use sni field, not separate grpcHost
